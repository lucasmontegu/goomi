...
export function Button({ title, onPress, icon, variant = 'lime', disabled, style }: { title: string; onPress: () => void; icon?: IconName; variant?: 'lime' | 'dark' | 'ghost'; disabled?: boolean; style?: StyleProp<ViewStyle> }) {
  const t = useTheme();
  const background = variant === 'lime' ? palette.lime : variant === 'dark' ? t.inverse : t.soft;
  const color = variant === 'lime' ? palette.ink : variant === 'dark' ? t.onInverse : t.text;
  return <Tactile label={title} disabled={disabled} onPress={onPress} style={[styles.button, { backgroundColor: background }, style]}><Txt weight="bold" size={15} color={color}>{title}</Txt>{icon && <Icon name={icon} color={color} size={20} />}</Tactile>;
}
export function CircleButton({ icon, onPress, dark, label }: { icon: IconName; onPress: () => void; dark?: boolean; label: string }) {
  const t = useTheme();
  // `dark` forces the immersive charcoal surface (challenge, paywall) regardless of theme.
  const background = dark ? '#292D24' : t.soft;
  return <Tactile label={label} onPress={onPress} style={[styles.circle, { backgroundColor: background }]}><Icon name={icon} color={dark ? palette.ivory : t.text} /></Tactile>;
}
...
export function Header({ title, subtitle, onClose, dark }: { title?: string; subtitle?: string; onClose?: () => void; dark?: boolean }) {
  const t = useTheme();
  return <View style={styles.header}>
    <CircleButton icon={onClose ? 'close' : 'arrow-back'} onPress={onClose ?? (() => router.back())} dark={dark} label={onClose ? 'Close' : 'Back'} />
    <View style={{ flex: 1 }}>
      {title && <Txt weight="bold" color={dark ? palette.ivory : t.text} style={{ textAlign: 'center' }}>{title}</Txt>}
      {subtitle && <Txt size={11} color={dark ? '#A6ACA0' : t.muted} style={{ textAlign: 'center' }}>{subtitle}</Txt>}
    </View>
    <View style={{ width: 44 }} />
  </View>;
}
